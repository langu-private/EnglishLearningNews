...
<head>
    ...
    <style>
        ...
        .header-line {{ font-weight: bold; color: #4B5563; margin-top: 2rem; padding-bottom: 0.5rem; text-align: center; font-size: 1.2rem; }}
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {{
            ...
            lines.forEach(line => {{
                ...
                if (btn) {{
                    const match = btn.getAttribute('onclick').match(/playShadowing\(([\\d.]+),\\s*([\\d.]+)\\)/);
                    if (match) {{
                        segments.push({{
                            element: line,
